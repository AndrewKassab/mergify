...
class DatabaseTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cur.execute("INSERT INTO Users (username, auth_token) VALUES ('%s', '%s')" % (cls.username, cls.auth_token))
    ...
    def test_add_new_user(self):
        ...
        self.db.add_new_entry_to_users(test_username, test_auth_token, test_access_token, test_refresh_token)
        con = sql.connect(db_path)
        cur = con.cursor()
        cur.execute("SELECT * FROM Users WHERE username = '%s'" % test_username)
        rows = cur.fetchall()
        user_id = rows[0][0]
        items = [rows[0][1], rows[0][2]]
        cur.execute("SELECT * FROM Tokens WHERE user_id = '%s'" % user_id)
        rows = cur.fetchall()
        items.extend([rows[0][1], rows[0][3]])
        con.close()
        expected_items = [test_username, test_auth_token, test_access_token, test_refresh_token]
        self.assertEqual(expected_items, items)

    def test_get_access_token_for_user(self):
        access_token = self.db.get_access_token_for_user(self.user_id)
        self.assertEqual(self.access_token, access_token)

    def test_get_refresh_token_for_user(self):
        refresh_token = self.db.get_refresh_token_for_user(self.user_id)
        self.assertEqual(self.refresh_token, refresh_token)

    def test_update_access_token_for_user(self):
        new_access_token = "new_access_token"
        prev_expiration_time = self.db.get_expiration_time_for_user(self.user_id)
        self.db.update_access_token_for_user(self.user_id, new_access_token)
        access_token = self.db.get_access_token_for_user(self.user_id)
        new_expiration_time = self.db.get_expiration_time_for_user(self.user_id)
